...
Falls man nur aus einem Elternteil ein Nachkomme erzeugen soll, wird dabei die Rekombination übersprungen und es wird mit der Mutation fortgefahren. 
Es gibt zwei Möglichkeiten die Nachfolger der zwei Eltern zu erzeugen. 
Bei der \textit{diskreten Rekombination} wählt man die Werte eines Gens für das Nachkommen aus, die besser geeignet sind. 
Am Beispiel \autoref{tabelleBeispiel} wurden nur die kleineren Werte für das Nachkommen bevorzugt. 
Die andere Möglichkeit ist die \textit{intermediäre Rekombination}.
Aus den Attributen der Elternteile wird der Durchschnitt gebildet und an das Nachkommen vererbt. 
%anhand der Gene beider Elternteile wird der Durchschnitt von beiden Werten berechnet und als Gen für den Nachkommen weitergegeben.

\begin{table}[]
\centering
\caption{Dirskete und intermediäre Rekombination}
...
Beim \textit{crossover} erhält man aus zwei Eltern zwei Nachkommen. 
Das Single-point crossover überschneidet die beiden Eltern an einer Stelle und die resultierenden Eltern werden als Nachkommen übernommen.
