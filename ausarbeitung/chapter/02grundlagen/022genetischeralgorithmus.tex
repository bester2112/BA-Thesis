...
%// Die Evolution{\"a}ren Algorithmen sind stochastische Optimierungsverfahren. Mithilfe eines Evolution{\"a}ren Algorithmus (EA) findet man nicht die beste L{\"o}sung f{\"u}r ein Problem, jedoch findet man eine gute L{\"o}sung, die der eigentlichen L{\"o}sung recht nahe kommt.
...
%, diese werden anhand des Fitnesswerts der Individuen aus einer Population bestimmt. 
...
Man f{\"u}hrt f{\"u}r eine Population die Selektion, Rekombination und Mutation so oft aus, bis man die gleiche Anzahl an Individuen f{\"u}r die n{\"a}chste Generation erzeugt hat.
...
%Dabei sieht eine bin{\"a}re Repr{\"a}sentation wie in \autoref{fig:binar8} aus.
...
Falls man nur aus einem Elternteil ein Nachkomme erzeugen soll, wird dabei die Rekombination {\"u}bersprungen und es wird mit der Mutation fortgefahren. 
Es gibt zwei M{\"o}glichkeiten die Nachfolger der zwei Eltern zu erzeugen. 
Bei der \textit{diskreten Rekombination} w{\"a}hlt man die Werte eines Gens f{\"u}r das Nachkommen aus, die besser geeignet sind. 
Am Beispiel \autoref{tabelleBeispiel} wurden nur die kleineren Werte f{\"u}r das Nachkommen bevorzugt. 
Die andere M{\"o}glichkeit ist die \textit{intermedi{\"a}re Rekombination}.
Aus den Attributen der Elternteile wird der Durchschnitt gebildet und an das Nachkommen vererbt. 
%anhand der Gene beider Elternteile wird der Durchschnitt von beiden Werten berechnet und als Gen f{\"u}r den Nachkommen weitergegeben.

\begin{table}[]
\centering
\caption{Beispiel f{\"u}r eine diskrete und intermedi{\"a}re Rekombination}
\label{tabelleBeispiel}
\begin{tabular}{l|llll}
Elternteil A          & 0,3  & 0,9  & 0,1  & 0,5 \\
Elternteil B          & 0,6  & 0,2  & 0,8  & 0,5 \\ \hline
Nachkomme dirket      & 0,3  & 0,2  & 0,1  & 0,5 \\ \hline
Nachkomme intermedi{\"a}r & 0,45 & 0,55 & 0,45 & 0,5
\end{tabular}
\end{table}

Beim \textit{crossover} erh{\"a}lt man aus zwei Eltern zwei Nachkommen. 
Das Single-point crossover {\"u}berschneidet die beiden Eltern an einer Stelle und die resultierenden Eltern werden als Nachkommen {\"u}bernommen.
Beim Two-point crossover hat man zwei {\"U}berschneidungen der Eltern, sowie man bei dem uniform corssover jedes zweite Element mit dem Partner getauscht hat. 

\begin{figure}[htbp] 
	\centering
	\begin{minipage}[t]{0.95\textwidth}
		\includegraphics[width=\textwidth]{pics/grundlage/{singlepoint}.png}
	\end{minipage}
	\begin{minipage}[t]{0.95\textwidth}
		\includegraphics[width=\textwidth]{pics/grundlage/{twopoint}.png}
	\end{minipage}
	\begin{minipage}[t]{0.95\textwidth}
		\includegraphics[width=\textwidth]{pics/grundlage/{uniform}.png}
	\end{minipage}
	\caption{Darstellung eines Single-point, Two-point und uniform crossovers (von oben nach unten)}
	\label{fig:MusterBild}
\end{figure}


\paragraph{Baumstrukturen}
